...
def process_image(event: func.EventGridEvent):
    ...
    blob_name = (
        blob_url.split("/image-uploads/")[-1]
        if "/image-uploads/" in blob_url
        else "unknown"
    )

    logging.info(
        "Processing image: %s (type: %s, size: %d bytes)",
        blob_name,
        content_type,
        content_length,
    )
...
def audit_log(event: func.EventGridEvent):
    ...
    entity = {
        ...
        "EventType": event.event_type,
        "EventTime": (
            event.event_time.isoformat() if event.event_time else now.isoformat()
        ),
...
def health(req: func.HttpRequest) -> func.HttpResponse:
    return func.HttpResponse(
        body=json.dumps({"status": "healthy", "service": "PhotoPipe Function App"}),
        mimetype="application/json",
        status_code=200,
    )
